...
class LinearizedConv3dProjection(nn.Module):
    """Conv3d patch projection expressed as a linear op with Conv3d-shaped state."""
    ...
    def __init__(self, conv3d: nn.Conv3d):
        ...
        self.weight = nn.Parameter(conv3d.weight.detach().clone(), requires_grad=conv3d.weight.requires_grad)
        if conv3d.weight.grad is not None:
            self.weight.grad = conv3d.weight.grad.detach().clone()

        if conv3d.bias is None:
            self.register_parameter("bias", None)
        else:
            self.bias = nn.Parameter(conv3d.bias.detach().clone(), requires_grad=conv3d.bias.requires_grad)
            if conv3d.bias.grad is not None:
                self.bias.grad = conv3d.bias.grad.detach().clone()
...
class LinearizedConv3dPatchEmbed(nn.Module):
    """Drop-in replacement for Qwen Conv3d patch embed when kernel equals stride."""
    ...
    def from_patch_embed(cls, patch_embed: nn.Module) -> "LinearizedConv3dPatchEmbed":
        ...
        linearized.train(patch_embed.training)
        return linearized
...
def linearize_qwen_vision_patch_embed(model: nn.Module) -> bool:
    """Replace nested Qwen vision patch Conv3d with an equivalent linear module."""
    try:
